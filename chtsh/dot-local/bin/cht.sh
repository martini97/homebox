...
CHTSH_CONF="${CHTSH_CONF:-$CHTSH_HOME/cht.sh.conf}"
CHTSH_URL="${CHTSH_URL:-https://cht.sh}"
CHTSH_CURL_OPTIONS="${CHTSH_CURL_OPTIONS:-}"
CHTSH_QUERY_OPTIONS="${CHTSH_QUERY_OPTIONS:-}"
...
CHTSH_MODE="$(cat "$CHTSH_HOME"/mode 2>/dev/null || true)"
[ "$CHTSH_MODE" != lite ] && CHTSH_MODE=auto
CHEATSH_INSTALLATION="$(cat "$CHTSH_HOME/standalone" 2>/dev/null || true)"
