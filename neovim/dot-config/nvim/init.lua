...
do -- sessions
	...
	vim.g.session_dir = vim.fs.joinpath(vim.fn.stdpath("data"), "sessions")

	if not vim.uv.fs_stat(vim.g.session_dir) then
		vim.uv.fs_mkdir(vim.g.session_dir, tonumber("755", 8))
	end

	local function get_current_session_file()
		local root = assert(vim.uv.cwd())
		local root_id = vim.fn.fnamemodify(root, ":t") .. "_" .. vim.fn.sha256(root):sub(1, 8) .. ".vim"
		return vim.fs.joinpath(vim.g.session_dir, root_id)
	end
	...
	vim.api.nvim_create_autocmd("VimEnter", { group = group, callback = load_session })
end
